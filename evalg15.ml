...
let rec sem ex en  = match (typeinf ex) with
  ...
  | False -> Bool false
  | Empty -> List []
  | Sum (a,b) -> (let c = sem a en  in
    let d = sem b en  in
    match c,d with
      Undefined, _
    | _, Undefined -> Undefined
    | _ -> Int (valutaInt c + valutaInt d))
  | Diff (a,b) -> (let c = sem a en  in
    let d = sem b en  in
    match c,d with
      Undefined, _
    | _, Undefined -> Undefined
    | _ -> Int (valutaInt c - valutaInt d))
  | Times (a,b) -> (let c = sem a en  in
    let d = sem b en  in
    match c,d with
      Undefined, _
    | _, Undefined -> Undefined
    | _ -> Int (valutaInt c * valutaInt d))
  | And (a,b) -> (let c = sem a en  in
    let d = sem b en  in
    match c,d with
      Undefined, _
    | _, Undefined -> Undefined
    | _ -> Bool (valutaBool c && valutaBool d))
  | Or (a,b) -> (let c = sem a en  in
    let d = sem b en  in
    match c,d with
      Undefined, _
    | _, Undefined -> Undefined
    | _ -> Bool (valutaBool c || valutaBool d))
  | Not a -> (let b = sem a en  in
    match b with
      Undefined -> Undefined
    | _ -> Bool (not (valutaBool (sem a en ))))
  | Eq (a,b) -> Bool (sem a en  = sem b en )
  | Less (a,b) -> Bool (valutaInt (sem a en ) < valutaInt (sem b en ))
  | Cons (a,b) -> (let c = sem b en in match c with
      Undefined -> Undefined
    | _ -> List ([sem a en] @ valutaList (sem b en)))
  | Head l -> (match (sem l en ) with
      List a when List.length a != 0 -> List.hd a
    | _ -> failwith "Inserire una lista non vuota.")
  | Tail l -> (match (sem l en ) with
      List a -> (match a with
	hd::tl -> List tl
      | _ -> failwith "Lista vuota.")
    | _ -> failwith "Inserire una lista.")
  | Fst p -> (match (sem p en ) with
      Pair (a,b) -> a
    | _ -> failwith "Fst opera solo su coppie.")
  | Snd p -> (match (sem p en ) with
      Pair (a,b) -> b
    | _ -> failwith "Snd opera solo su coppie.")
  | Epair (a,b) -> Pair (sem a en , sem b en )
  | Ifthenelse (a,b,c) -> if(valutaBool (sem a en))
      then sem b en
      else sem c en 
  | Let (i,a,b) -> let c = sem a en in let d = bind (en,i,c) in sem b d
  | Fun (i,a) -> (let r = sem a en  in
  ...
  | Appl (a,b) -> let c = sem a en in let d = sem b en in (match c with
      Closure (f,e) -> (match f with
	Fun(x,t) -> let g = bind (e,x,d) in sem t g (*bind en,x,d*)
      | _ -> failwith "??? apply")
    | Undefined -> c
    | _ -> failwith "Apply si aspetta una funzione!")
  | Rec (i,a) -> let b = sem a en in match b with
